...
class ProxyService {
    static parseErrorResponse(proxyRes, url, method) {
        const cleanRes = {
            ...
            proxyRes
                ...
                try {
                    if ('content-encoding' in proxyRes.headers && (/gzip/i.test(proxyRes.headers['content-encoding']))) {
                        cleanRes['body'] = zlib_1.gunzipSync(buffer).toString('utf8');
                    }
                    else {
    ...
    static parseProxyCookie(cookieString) {
        ...
        const config = [expPart, pathPart, domainPart].filter(v => !!v && v.length)
            .reduce((prev, curr) => {
        ...
        const defaultExpires = (new Date().valueOf() + (1000 * 60 * 60 * 24 * 7));
        config.expires = new Date(config.expires || defaultExpires);
        return {
            name,
            value: cookieValue,
            options: Object.assign({}, config, { httpOnly })
        };
    }
    static formatAuthCookie(existingCookies, name, value, blacklist) {
        let cookieMap = {};
